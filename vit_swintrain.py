...
class Dual_model(nn.Module):
    def __init__(self, args):
        ...
        self.vit_base, feat_dim = _build_swin_model('swinb_imagenet22k_224', 224, './backbone_ckpt')
        for k, p in self.vit_base.named_parameters():
            name_list = k.split('.')
            if name_list[1] == 'deep_ppt' or name_list[1] == 'proj_ppt':
                p.requires_grad = True
            elif name_list[1] == '2' and name_list[2] == 'cross_attn' and name_list[4] in ['ffn', 'ffn_norm']:
                p.requires_grad = True
            else:
                p.requires_grad = False
        self.semantic_prompt = SemanticPromptModule()
        self.reweight_adapter = ReWeightingAdapter(1024, args.nb_classes)
        self.class_head = nn.Linear(1024, args.nb_classes, bias=True)
        trunc_normal_(self.class_head.weight, std=0.02)

    def forward(self, x):
        low_level_prompt = self.semantic_prompt(x)
        x, p = self.vit_base.forward_features(x, low_level_prompt=low_level_prompt)
        x = self.reweight_adapter(x)
        p = self.reweight_adapter(p)
        return self.class_head(x), self.class_head(p)


class SemanticPromptModule(nn.Module):
    def __init__(self):
        super(SemanticPromptModule, self).__init__()
        self.color_hist_fc = nn.Linear(256, 128)
        self.texture_fc = nn.Linear(256, 128)
        self.shape_fc = nn.Linear(256, 128)

    def forward(self, x):
        color_hist = self.extract_color_histogram(x)
        texture = self.extract_gabor_texture(x)
        shape = self.extract_sobel_shape(x)
        color_feat = self.color_hist_fc(color_hist)
        texture_feat = self.texture_fc(texture)
        shape_feat = self.shape_fc(shape)
...
def get_args():
    parser = argparse.ArgumentParser('Semantic Prompt Tuning', add_help=False)
...
def main(args):
    ...
    data_loader_train = torch.utils.data.DataLoader(dataset_train, sampler=sampler_train, batch_size=args.batch_size, drop_last=True)
    data_loader_val = torch.utils.data.DataLoader(dataset_val, sampler=sampler_val, batch_size=args.batch_size, drop_last=False)
    model = Dual_model(args).to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = create_optimizer(args, model)
    for epoch in range(args.epochs):
        train_one_epoch(model, criterion, data_loader_train, optimizer, device, epoch, None)
